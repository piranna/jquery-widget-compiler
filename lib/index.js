...
function processors(name, options)
{
  ...
  var result =
  {
    ...
    {
      var pathName = path.resolve(options.input, 'html', name+'.html')
...
//        if(prefix) name = prefix + "." + name;
    ...
    {
      var pathName = path.resolve(options.input, 'js', name+'.js')
...
{
  var name = path.basename(options.input, ".html");

  async.parallel(processors(name, options), function(error, results)
  {
    if(error) return callback(error)
...
function directory(options, callback)
{
  function mapFiles(filename, callback)
  {
    ...
    async.parallel(processors(name, options), callback)
  }
  ...
  fs.readdir(options.input+'/html', function(error, files)
  {
    if(error) return callback(error)
